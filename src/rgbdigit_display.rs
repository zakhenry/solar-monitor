...
impl SolarStatusDisplay for RgbDigitDisplay<'_> {
    ...
    fn show_status(&mut self, status: SolarStatus) -> Result<(), SolarMonitorError> {
        ...
        if status.battery_power_watts > -100 { // epsilon to stop it from flickering while around zero
        ...
        if status.grid_power_watts > 100 { // epsilon to stop it from flickering while around zero
